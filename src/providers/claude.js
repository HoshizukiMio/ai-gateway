 ...
 * @param {Object} requestBody - Request body (already in Claude format)
 * @param {string} apiKey - User's API key
 * @param {string} endpoint - API endpoint
 * @returns {Promise<Response>} API response
 */
export async function makeRequest(requestBody, apiKey, endpoint = '/v1/messages') {
    const config = getProvider('claude');

    // Build request URL
    const url = config.baseURL + endpoint;

    // Prepare headers - Claude requires specific headers
    const headers = {
